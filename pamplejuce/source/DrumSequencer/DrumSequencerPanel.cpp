...
void DrumSequencerPanel::resized()
{
    ...
    int stepGridHeight = area.getHeight() - static_cast<int> (trackRows.size()) * 26 - 30;
    if (stepGridHeight < 60) stepGridHeight = 60;
    int trackRowHeight = stepGridHeight / DRUM_TRACK_COUNT;
    ...
    for (int t = 0; t < DRUM_TRACK_COUNT; ++t)
    {
        if (t < static_cast<int> (trackRows.size()))
            trackRows[static_cast<size_t> (t)]->setBounds (area.removeFromTop (trackRowHeight));
        auto row = area.removeFromTop (trackRowHeight);

        for (int s = 0; s < DRUM_STEPS; ++s)
        {
            stepButtons[t][s].setBounds (stepArea.removeFromLeft (stepWidth).reduced (1, 1));
        }

        row.removeFromLeft (stepArea.getWidth());
    }
